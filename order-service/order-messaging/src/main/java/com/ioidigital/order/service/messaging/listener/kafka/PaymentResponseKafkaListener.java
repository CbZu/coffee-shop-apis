...
import com.ioidigital.order.service.domain.ports.input.message.listener.payment.PaymentResponseMessageListener;
import com.ioidigital.kafka.consumer.KafkaConsumer;
import com.ioidigital.kafka.order.avro.model.PaymentResponseAvroModel;
import com.ioidigital.kafka.order.avro.model.PaymentStatus;
        ...
        messages.forEach(paymentResponseAvroModel -> {
            try {
                if (PaymentStatus.COMPLETED == paymentResponseAvroModel.getPaymentStatus()) {
                    ...
                    // paymentCompleted handle
                } else if (PaymentStatus.CANCELLED == paymentResponseAvroModel.getPaymentStatus() ||
                        PaymentStatus.FAILED == paymentResponseAvroModel.getPaymentStatus()) {
                    log.info("Processing unsuccessful payment for order id: {}", paymentResponseAvroModel.getOrderId());
                    // paymentCancelled handle
                }
